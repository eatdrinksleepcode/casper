...
%myDir%nuget sources Remove -Name "Casper Dev" -ConfigFile %localNugetConfig% >NULL 2>&1
%myDir%nuget sources Add -Name "Casper Dev" -Source %myDir%\out\packages -ConfigFile %localNugetConfig% >NULL || goto :eof
REM Adding any source to a NuGet config also adds the default source, which we don't want for this operation
%myDir%nuget sources Disable -Name "https://www.nuget.org/api/v2/" -ConfigFile %localNugetConfig% >NULL || goto :eof
